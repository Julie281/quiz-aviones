...
function buildAircraftQuestions(plane) {
...

  {
    ...
    const { options, correctIndex } = buildChoiceSet(correct, distractors, state.aircraftDb.map((item) => item.name));
    questions.push({ id: `${plane.id}-image`, type: "image", tag: "Imagen", prompt: "¿Qué avión aparece en la imagen?", image: plane.image, options, correctIndex });
  }

  {
    ...
    questions.push({ id: `${plane.id}-manufacturer`, type: "manufacturer", tag: "Fabricante", prompt: `¿Quién fabrica el ${plane.name}?`, image: plane.image, options, correctIndex });
  }

  {
    ...
    const { options, correctIndex } = buildChoiceSet(correct, distractors);
    questions.push({ id: `${plane.id}-domain`, type: "domain", tag: "Ámbito", prompt: `¿El ${plane.name} pertenece al ámbito civil, militar o prototipo?`, image: plane.image, options, correctIndex });
  }

  {
    ...
    const { options, correctIndex } = buildChoiceSet(correct, distractors);
    questions.push({ id: `${plane.id}-role`, type: "role", tag: "Rol", prompt: `¿Cuál es el rol principal del ${plane.name}?`, image: plane.image, options, correctIndex });
  }

  {
    ...
    const { options, correctIndex } = buildChoiceSet(correct, distractors);
    questions.push({ id: `${plane.id}-decade`, type: "decade", tag: "Década", prompt: `¿En qué década encaja mejor el origen del ${plane.name}?`, image: plane.image, options, correctIndex });
  }

  {
    ...
    const { options, correctIndex } = buildChoiceSet(correct, distractors);
    questions.push({ id: `${plane.id}-country`, type: "country", tag: "Origen", prompt: `¿Con qué país o área se asocia el ${plane.name}?`, image: plane.image, options, correctIndex });
  }

  {
    ...
    const { options, correctIndex } = buildChoiceSet(correct, distractors);
    questions.push({ id: `${plane.id}-propulsion`, type: "propulsion", tag: "Propulsión", prompt: `¿Qué tipo de propulsión utiliza el ${plane.name}?`, image: plane.image, options, correctIndex });
  }
  ...
  if (typeof plane.engines === "number") {
    ...
    const { options, correctIndex } = buildChoiceSet(correct, distractors);
    questions.push({ id: `${plane.id}-engines`, type: "engines", tag: "Motores", prompt: `¿Cuántos motores tiene el ${plane.name}?`, image: plane.image, options, correctIndex });
  }

  if (typeof plane.seats === "number") {
    const correct = String(plane.seats);
    const distractors = getClosestNumbers(plane.seats, state.aircraftDb.map((item) => item.seats), 3).map(String);
    const allSeats = state.aircraftDb.map((item) => item.seats).filter((value) => typeof value === "number").sort((a, b) => a - b).map(String);
    const { options, correctIndex } = buildChoiceSet(correct, distractors, allSeats);
    questions.push({ id: `${plane.id}-seats`, type: "seats", tag: "Plazas", prompt: `¿Qué cifra de plazas o pasajeros se usa para el ${plane.name}?`, image: plane.image, options, correctIndex });
  }

  if (plane.cruiseLabel) {
    const correct = plane.cruiseLabel;
    const distractors = state.aircraftDb.map((item) => item.cruiseLabel).filter((value) => value && value !== correct);
    const { options, correctIndex } = buildChoiceSet(correct, [...new Set(distractors)]);
    questions.push({ id: `${plane.id}-speed`, type: "speed", tag: "Velocidad", prompt: `¿Qué velocidad de crucero se usa para el ${plane.name}?`, image: plane.image, options, correctIndex });
  }

  if (typeof plane.rangeNm === "number") {
    const correct = formatNm(plane.rangeNm);
    const distractors = getClosestNumbers(plane.rangeNm, state.aircraftDb.map((item) => item.rangeNm), 3).map(formatNm);
    const allRanges = state.aircraftDb.map((item) => item.rangeNm).filter((value) => typeof value === "number").sort((a, b) => a - b).map(formatNm);
    const { options, correctIndex } = buildChoiceSet(correct, distractors, allRanges);
    questions.push({ id: `${plane.id}-range`, type: "range", tag: "Alcance", prompt: `¿Qué alcance en millas náuticas se usa para el ${plane.name}?`, image: plane.image, options, correctIndex });
  }
...
const LIGHT_QUESTIONS = [
  { id: "lights-left-wing", type: "lights", tag: "Luces", prompt: "¿Qué color lleva la luz de posición del ala izquierda?", image: null, options: ["Roja", "Verde", "Blanca", "Ámbar"], correctIndex: 0 },
  { id: "lights-right-wing", type: "lights", tag: "Luces", prompt: "¿Qué color lleva la luz de posición del ala derecha?", image: null, options: ["Verde", "Roja", "Blanca", "Azul"], correctIndex: 0 },
  { id: "lights-tail", type: "lights", tag: "Luces", prompt: "¿Dónde va normalmente la luz blanca de posición?", image: null, options: ["En la cola", "En el ala izquierda", "En el morro", "En el tren principal"], correctIndex: 0 },
  { id: "lights-anticollision", type: "lights", tag: "Luces", prompt: "¿Qué luces sirven principalmente para hacer más visible el avión y evitar colisiones?", image: null, options: ["Las luces anticolisión o estrobos", "Solo las luces de cabina", "Solo las luces de taxi", "Solo las luces del panel"], correctIndex: 0 }
];
...
function renderImage(question) {
  ...
  els.planeImage.onerror = () => {
    ...
    els.imageFallback.textContent =
      `No se encontró la imagen:\n${question.image}\n\nAñade ese archivo dentro de la carpeta images/.`;
  };
  ...
  els.planeImage.src = question.image;
}
